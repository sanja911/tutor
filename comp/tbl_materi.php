  ...
  <tbody>
    <?php
    $query = mysql_query ("SELECT * FROM user INNER JOIN materi_tutor on user.id_user=materi_tutor.id_tutor  INNER JOIN mapel ON materi_tutor.id_mapel=mapel.id_mapel where user.id_user='$out[id_user]' order by user.id_user ASC");
      if($query == false){
      ...
      while ($ar = mysql_fetch_array ($query)){
        ...
        ?>
          <tr>
            ...
            <td align="center"><a href="../comp/user_data/<?php echo $ar['file'];?>"><?php echo $ar['file'];?></a></td>
            <?php echo"
            <td align=center>
